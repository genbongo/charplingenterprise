    ...
    $('#addProduct').on('click', function(){
        var newProduct = `<div class="row mt-2" id="row-${productsNum+1}">
            ...
            <div class="col-lg-4 pl-3 pt-1">
                <a href="javascript://;" class="removeRow" row-id="row-${productsNum+1}">X</a>
            </div>
        </div>`;
        productsNum++;
        ...
        const sizeSelect = $(this).parent().siblings()[0].children[0].id
         ...
         $.get("{{ url('get-sizes') }}" + '/' + id, function (data) {
            ...
            $.each(data, function(key, row){
                $(`#${sizeSelect}`).append(`<option data-price="${row.price}" value="${row.size +'-'+ row.price+'-'+ row.id }" value="${row.size}">${row.size}</option>`)
            })
